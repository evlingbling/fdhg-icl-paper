...
ROOT = Path(
    "results/rel_trial_studies_has_dmc_matched"
)

OUT = Path(
    "results/rel_trial_studies_has_dmc_multiseed"
)
...
y_train = normalize_binary(train_clean["has_dmc"])
y_val = normalize_binary(val_clean["has_dmc"])
...
gate_summary = pd.DataFrame([{
    ...
    "average_precision_delta_std": float(
        gate["average_precision_delta"].std(
            ddof=1
        )
    ),
